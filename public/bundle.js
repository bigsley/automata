...
/***/ function(module, exports, __webpack_require__) {
	...
	var GridInputController = _react2.default.createClass({
	  displayName: 'GridInputController',
	  ...
	  heightChange: function heightChange(event) {
	    var height = parseInt(event.target.value) || 0;
	    this.setState({ height: height });
	  },
	  widthChange: function widthChange(event) {
	    var width = parseInt(event.target.value) || 0;
	    this.setState({ width: width });
	  },
	  frequencyChange: function frequencyChange(event) {
	    var frequency = parseInt(event.target.value) || 0;
	    this.setState({ frequency: frequency });
	  },
	  bornChange: function bornChange(event) {
	    var born = event.target.value;
	    var bornArray = born.split(',').map(function (x) {
	      return parseInt(x);
	    });
	    this.setState({
	      born: born,
	  ...
	  liveChange: function liveChange(event) {
	    var live = event.target.value;
	    var liveArray = live.split(',').map(function (x) {
	      return parseInt(x);
	    });
	    this.setState({
	      live: live,
	  ...
	  marginChange: function marginChange(event) {
	    var margin = event.target.value || 0;
	    this.setState({ margin: margin });
	  },
	  sizeChange: function sizeChange(event) {
	    var size = event.target.value || 0;
	    this.setState({ size: size });
	  },
	  render: function render() {
	    var _state = this.state;
	    var bornArray = _state.bornArray;
	    var liveArray = _state.liveArray;
	    var height = _state.height;
	    var width = _state.width;
	    var frequency = _state.frequency;
	    var born = _state.born;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var constructGrid = function constructGrid(width, height) {
	  var grid = [];
	...
	var nextGrid = function nextGrid(grid, born, live) {
	  ...
	  for (var y = 0; y < grid.length; y++) {
	    ...
	    for (var x = 0; x < grid[0].length; x++) {
	      ...
	      for (var checkX = x - 1; checkX < x + 2; checkX++) {
	        for (var checkY = y - 1; checkY < y + 2; checkY++) {
	          var val = grid[checkY] && grid[checkY][checkX] ? 1 : 0;
	      ...
	      var memoizeVal = memoizeMap[memoizeIndex];
	...
	var GridController = _react2.default.createClass({
	  ...
	  getInitialState: function getInitialState() {
	    ...
	    var grid = constructGrid(width, height);
	  ...
	  componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
	    var born = nextProps.born;
	    var frequency = nextProps.frequency;
	    var live = nextProps.live;
	    var height = nextProps.height;
	    var width = nextProps.width;
	    var _props2 = this.props;
	    var oldBorn = _props2.born;
	    var oldFrequency = _props2.frequency;
	    var oldLive = _props2.live;
	  ...
	  _createInterval: function _createInterval(someProps) {
	    var _this = this;

	    count = 0;
	    time = Date.now();
	    var animateLoop = function animateLoop() {
	      var born = someProps.born;
	      var live = someProps.live;


	      var newGrid = nextGrid(_this.state.grid, born, live);

	      count++;
	      var timePerStep = (Date.now() - time) / count;
	    ...
	    var msPerLoop = 1000.0 / someProps.frequency;
	    this.setState({ animateInterval: setInterval(animateLoop, msPerLoop) });
	  },
	  _destroyInterval: function _destroyInterval() {
	    var animateInterval = this.state.animateInterval;


	    clearInterval(animateInterval);
	    this.setState({ animateInterval: null });
	  },
	  _resetInterval: function _resetInterval(nextProps) {
	    if (this.state.animateInterval) {
	      this._destroyInterval();
	      this._createInterval(nextProps);
	    }
	  },
	  handleRandomize: function handleRandomize() {
	  ...
	  render: function render() {
	    return _react2.default.createElement(
	      ...
	      _react2.default.createElement(
	        'div',
	        null,
	        Math.round(this.state.timePerStep || 0),
	        ' ms/step'
	      )
	    );
	  }
